...
KinematicsNode::KinematicsNode() : nh("~")
{
    ...
    nh.getParam("output_topic", this->pub_topic);
}
...
void KinematicsNode::run()
{
    ...
    this->odometry_publisher = nh.advertise<nav_msgs::Odometry>(pub_topic, 1000);
	ros::spin();
}
...
void KinematicsNode::steering_measurement_callback(const atv_can::SteeringMeasurement& measurement) 
{
    ...
    //temp = steering_angle_radians * 57.3;
    //ROS_DEBUG("Steering Angle: "<<temp);
