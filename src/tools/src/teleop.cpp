...
void TeleopVehicle::keyLoop() {
	...
	for (;;) {
		for (int clearint = 0; clearint < 3; clearint++) {
			std::cout << " " << std::endl;
		}
		...
		if (mtr_mode) {
			std::cout << "Torque %: " << ctrl_ref << ", Turning reference: "
					<< turn_ref << std::endl;
			std::cout << "Torque delta %: " << ctrl_ref_delta
					<< ", Turning reference delta: " << turn_ref_delta
					<< std::endl;
		} else {
			std::cout << "Velocity mm/s: " << ctrl_ref
					<< ", Turning reference: " << turn_ref << std::endl;
			std::cout << "Velocity delta mm/s: " << ctrl_ref_delta
					<< ", Turning reference delta: " << turn_ref_delta
					<< std::endl;
		}
		...
		case KEYCODE_UP:
			ROS_DEBUG("UP");
			ctrl_ref = minbound(ctrl_ref + ctrl_ref_delta,
					mtr_mode * trq_max + !(mtr_mode) * vel_max);
			// detect = true;
			break;
		case KEYCODE_DOWN:
			ROS_DEBUG("DOWN");
			ctrl_ref = maxbound(ctrl_ref - ctrl_ref_delta,
					mtr_mode * trq_min + !(mtr_mode) * vel_min);
			// detect = true;
			break;
		case KEYCODE_RESET:
			ROS_DEBUG("RESET");
			...
			ctrl_ref_delta = mtr_mode * trq_scale_default
					+ !(mtr_mode) * vel_scale_default;
			detect = true;
			break;
		case KEYCODE_W:
			ROS_DEBUG("MOTOR CONTROL SCALE UP");
			ctrl_ref_delta = minbound(
					ctrl_ref_delta + mtr_mode * trq_scale_change
							+ !(mtr_mode) * vel_scale_change,
					mtr_mode * trq_scale_max + !(mtr_mode) * vel_scale_max);
			// detect = true;
			break;
		case KEYCODE_S:
			ROS_DEBUG("MOTOR CONTROL SCALE DOWN");
			ctrl_ref_delta = maxbound(
					ctrl_ref_delta - mtr_mode * trq_scale_change
							- !(mtr_mode) * vel_scale_change,
					mtr_mode * trq_scale_min + !(mtr_mode) * vel_scale_min);
			// detect = true;
			break;
		case KEYCODE_SPACE:
		...
		case KEYCODE_E:
			...
			turn_ref_delta = minbound(turn_ref_delta + trn_scale_change,
					trn_scale_max);
			// detect = true;
			break;
		case KEYCODE_D:
			ROS_DEBUG("ROTATION SCALE DOWN");
			turn_ref_delta = maxbound(turn_ref_delta - trn_scale_change,
					trn_scale_min);
			// detect = true;
			break;
		case KEYCODE_TAB:
		...
		if (detect == true) {
			...
			if (client.call(mesg)) {
				std::cout << "sent-received" << mesg.response.status
						<< std::endl;
				ROS_DEBUG("can_talker sent-received");
			} else {
			...
			std::cout << "NO ! sent-received" << std::endl;
