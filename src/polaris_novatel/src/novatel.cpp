...
inline void DefaultBestPositionCallback(Position best_position, double time_stamp){
	...
	std:: cout << "BESTPOS: \nGPS Week: " << best_position.header.gps_week <<
			...
			"  Differential Age: " << best_position.differential_age << std::endl <<
			"  Packet Rate : " << PacketRate << std::endl;
}
*/
...
bool Novatel::Connect(std::string port, int baudrate, bool search) {
	...
	bool connected = Connect_(port, baudrate, search);
	...
	if (!connected && search) {
		...
		for (int ii=0; ii<9; ii++){
			...
			if (Connect_(port, bauds_to_search[ii], search)) {
				found = true;
				break;
			}
		}
		...
		if (found) {
			...
			connected = Connect_(port, baudrate, search);
		} 
	}
...
bool Novatel::Connect_(std::string port, int baudrate=115200, bool search=true) {
	try {
		...
		//serial_port_->write("UNLOGALL COM2\r\n");
		log_info_("UNLOGALL Sent...");
		// wait for data to stop coming in
		boost::this_thread::sleep(boost::posix_time::milliseconds(1000));
		// clear serial port buffers
		serial_port_->flush();
		log_info_("Serial Port Flushed...");
		...
		while (ii<5) {
			try {
				...
				if (ack_condition_.timed_wait(lock,timeout)) {
					...
					log_error_("No acknowledgment received for log: " + *it);
				}
				ii++;
			} catch (std::exception &e) {
				std::stringstream output;
...
void Novatel::ConfigureInterfaceMode(std::string com_port,  
	...
	try {
		...
		serial_port_->write("INTERFACEMODE " + com_port + " " + rx_mode + " " + tx_mode + " " + "OFF" + "\r\n");
		// wait for acknowledgement (or 2 seconds)
		boost::mutex::scoped_lock lock(ack_mutex_);
		boost::system_time const timeout=boost::get_system_time()+ boost::posix_time::milliseconds(2000);
		if (ack_condition_.timed_wait(lock,timeout)) {
			...
			log_error_("No acknowledgment received for interface mode command.");
		}
	} catch (std::exception &e) {
		std::stringstream output;
...
void Novatel::ConfigureLeverArm(std::string xoff, std::string yoff,
		std::string zoff) {

	try {
		// ex: SETIMUTOANTOFFSET xoff yoff zoff
		log_info_("SETIMUTOANTOFFSET " + xoff + " " + yoff + " " + zoff + "\r\n");
		serial_port_->write("SETIMUTOANTOFFSET " + xoff + " " + yoff + " " + zoff + "\r\n");
		// wait for acknowledgment (or 2 seconds)
		boost::mutex::scoped_lock lock(ack_mutex_);
		boost::system_time const timeout=boost::get_system_time()+ boost::posix_time::milliseconds(2000);
		if (ack_condition_.timed_wait(lock,timeout)) {
			log_info_("Ack received for SETIMUTOANTOFFSET command.");
		} else {
			log_error_("No acknowledgment received for calibrate lever arm command.");
		}
	} catch (std::exception &e) {
		std::stringstream output;
		output << "Error issuing calibrate commands: " << e.what();
		log_error_(output.str());
	}
}
...
bool Novatel::UpdateVersion()
{
	...
	try {
		std::cout << "reading all existing data from the stream" << std::endl;
		// clear port
		serial_port_->flush();
		...
		std::cout << "received response: " << gps_response << std::endl;
...
void Novatel::ReadSerialPort() {
	...
	while (reading_status_) {
		try {
			// read data
			//len = serial_port_->read(buffer, 1);
...
void Novatel::BufferIncomingData(unsigned char *message, unsigned int length)
{
	...
	for (unsigned int ii=0; ii<length; ii++) {
		...
		if (buffer_index_ == 0) {	// looking for beginning of message
			...
			//std::cout << "Binary message id: " << message_id << std::endl;
...
void Novatel::ParseBinary(unsigned char *message, size_t length, BINARY_LOG_TYPE message_id) {
	...
	case BESTPOSB_LOG_TYPE:
		//std::cout << "BESTPOSB Received"<<std::endl;
		Position best_pos;
...
bool Novatel::ConvertUTMLLA(double northing, double easting,
		int zone, bool north, double *Lat, double *Long )
{

	const double a = 6378137.0; // Semi-major axis in meters.
	const double b = 6356752.3142; // Semi-minor axis in meters.
	const double f = 1 / 298.257223563; // Inverse flattening.
	const double Radius = 6371e3; // Radius of Earth.
	const double ee = 6.69437999014 * 0.001; // First Eccentricity Squared
	const double e2 = 6.73949674228 * 0.001; // Second Eccentricity Squared
	const double k0 = 0.9996; // UTM scale on the central meridian
	const double falseEasting = 500e3;
	const double falseNorthing = 10000e3;
	const double D2R = M_PI / 180.0;

	double sigma, TauI, deltaTauI; // Tau represents angle on conformal plane
	double TauPrime, TauIPrime;
	double x, y, xx, yy, yyP, xxP; //x is easting, y is northing
	double e, n, n2, n3, n4, n5, n6, A;
	int iter, k;
	double Beta[7] = {};

	x = easting - falseEasting; // make x ± relative to central meridian
	if (north == false)
	...
	n2 = n*n;
	n3 = pow(n,3); n4 = pow(n,4); n5 = pow(n,5); n6 = pow(n,6);
	A = a/(1+n)*(1 + 1/4*n2 + 1/64*n4 + 1/256*n6); // 2*pi*A is circumference of the meridian.
	...
	yyP = yy;
	for (k = 1; k<=6; k++)
		yyP = yyP - Beta[k] * sin(2*k*yy) * cosh(2*k*xx);

	xxP = xx;
	for (k = 1; k<=6; k++)
		xxP = xxP - Beta[k] * cos(2*k*yy) * sinh(2*k*xx);

	TauPrime = sin(yyP) / sqrt(sinh(xxP)*sinh(xxP) + cos(yyP)*cos(yyP));
	...
	do
	{
		sigma = sinh(e*atanh(e*TauI/sqrt(1+TauI*TauI)));
		TauIPrime = TauI * sqrt(1+sigma*sigma) - sigma * sqrt(1+TauI*TauI);
		deltaTauI = (TauPrime - TauIPrime)/sqrt(1+TauIPrime*TauIPrime)
					* (1 + (1-e*e)*TauI*TauI) / ((1-e*e)*sqrt(1+TauI*TauI));
		TauI = TauI + deltaTauI;
		iter = iter + 1;
	}while(fabs(deltaTauI) > 1e-12);

	*Lat = atan(TauI);
	*Long = atan2(sinh(xxP), cos(yyP)) + ((zone-1)*6.0 - 180.0 + 3.0)*D2R;
