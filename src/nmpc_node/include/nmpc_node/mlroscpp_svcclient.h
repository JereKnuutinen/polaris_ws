...
#define MATLABSvcClient_createSvcClient(obj, mlSvcName, mlSvcSize, isPersistent) \
    obj->createSvcClient(mlSvcName, mlSvcSize, isPersistent)
...
class MATLABSvcClient {
  ...
  public:
     ...
     * @param isPersistent - persistent connection to service server
     */
    void createSvcClient(const char* mlSvcName, size_t mlSvcSize, bool isPersistent) {
        std::string svcname(mlSvcName, mlSvcSize);
        ros::NodeHandle nh;
        client_ = nh.serviceClient<SvcType>(svcname, isPersistent);
    }
